...
<script src="plugins/chart.js/chartjs-plugin-labels.min.js"></script> <!-- script ekstensi  label chartjs -->
<script>
   $(function() {
...
<script>
   //pekerjaan
   $(function() {
      var pieChartCanvas = $('#asd').get(0).getContext('2d')
      var pieData = {
         labels: [
            <?php while ($row = mysqli_fetch_array($pekerjaan)) {
               echo '"' . $row['pekerjaan'] . '",';
            } ?>
         ],
         datasets: [{
